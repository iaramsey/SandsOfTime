...
PUB Extension(flag) | ArmSetPoint, HGCase, Startcnt,Ex_Dog,HG_Dog ' *** Extends the arm and flips the HG, also makes sure that the arm doesn't stray away from HG
  ...
  repeat until ina[FrontLimit] == 1 or Ex_Dog > 350      ' Go until the limit switch is triggered or until the watchdog expires (about 7.5 seconds)
...
PUB ResetExtension | Ret_Dog ' *** Retracts arm, simple
  ...
  Ret_Dog:=0
  repeat until ina[BackLimit] == 1 or Ret_Dog > 238
      EDutyCycle := 100
      waitcnt(clkfreq/50+cnt)
